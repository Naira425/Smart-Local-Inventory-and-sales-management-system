...
function recordSale(productId, quantity) {
    ...
    if (product.quantity === 0) return { success: false, message: `${product.name} is OUT OF STOCK!` };
    if (product.quantity < quantity) return { success: false, message: `Only ${product.quantity} units available.` };
    ...
    return { success: true, message: `Sold ${quantity} x ${product.name} for ${formatCurrency(product.price * quantity)}` };
}

function renderCashierRecordSale() {
    const products = getProducts();
    let html = `
        <div class="sale-form">
    ...
    document.getElementById('processSaleBtn').addEventListener('click', () => {
        ...
        const result = recordSale(productId, quantity);
...
function renderCashierTodaySales() {
    const todaySales = getTodaySales();
    const total = todaySales.reduce((s, r) => s + r.amount * r.quantity, 0);
    ...
    let html = `
        ...
        <div class="card">
            ...
            <div class="responsive-table">
                <table>
                    <thead>
                        <tr><th>Time</th><th>Product</th><th>Category</th><th>Quantity</th><th>Unit Price</th><th>Total</th></tr>
                    </thead>
                    <tbody>
                        ${todaySales.length === 0 ? '<tr><td colspan="6" style="text-align:center;">No sales recorded today</td></tr>' : 
                            todaySales.map(s => `
                                <tr>
                                    ...
                                    <td><strong>${formatCurrency(s.amount * s.quantity)}</strong></td>
                                </tr>
                            `).join('')
                        }
                    </tbody>
...
function renderCashierInventoryStatus() {
    const products = getProducts();
    ...
    let html = `
        ...
        <div class="card">
            ...
            <div class="responsive-table">
                <table>
                    <thead>
                        <tr><th>Product</th><th>Category</th><th>Unit Price</th><th>Current Stock</th><th>Supplier</th></tr>
                    </thead>
                    <tbody>
                        ${products.map(p => `
                            <tr>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('[data-cashier-view]').forEach(link => {
        link.addEventListener('click', () => {
            ...
            if (view === 'record-sale') {
                document.getElementById('cashierTitle').innerText = 'Record Sale';
                renderCashierRecordSale();
            } else if (view === 'today-sales') {
                document.getElementById('cashierTitle').innerText = 'Today\'s Sales';
                renderCashierTodaySales();
            } else if (view === 'inventory-status') {
                document.getElementById('cashierTitle').innerText = 'Inventory Status';
                renderCashierInventoryStatus();
            }
        });
    });
