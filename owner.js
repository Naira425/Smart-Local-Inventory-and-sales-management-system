...
function renderOwnerSalesSummary() {
    document.getElementById('ownerTitle').innerText = '📊 Sales Summary';
    const sales = getSales();
    const totalRevenue = sales.reduce((sum, s) => sum + (s.amount * s.quantity), 0);
    ...
    let html = `
        ...
        <div class="card">
            ...
            <div class="responsive-table">
                <table>
                    <thead>
                        <tr><th>Product</th><th>Category</th><th>Quantity</th><th>Amount</th><th>Date</th></tr>
                    </thead>
                    <tbody>
                        ${sales.slice(-8).reverse().map(s => `
                            <tr>
                                ...
                                <td>${s.category}</td>
                                <td>${s.quantity}</td>
                                <td>${formatCurrency(s.amount * s.quantity)}</td>
                                <td>${s.date}</td>
                            </tr>
                        `).join('')}
                    </tbody>
...
function renderOwnerInventorySummary() {
    ...
    let html = `
        ...
        <div class="card">
            ...
            <div class="responsive-table">
                <table>
                    <thead>
                        <tr><th>Product</th><th>Category</th><th>Quantity</th><th>Price</th><th>Value</th></tr>
                    </thead>
                    <tbody>
                        ${products.map(p => `
                            <tr>
...
function renderOwnerSupplierSummary() {
    ...
    let html = `
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-value">${supplierMap.size}</div>
            </div>
        </div>
    `;
    
    for(let [sup, items] of supplierMap.entries()) {
        html += `
            <div class="card">
                <h3><i class="fas fa-building"></i> ${sup}</h3>
                <p><i class="fas fa-envelope"></i> ${items[0].supplierContact}</p>
                ${items.map(i => `<div><strong>${i.name}</strong> - ${i.category} - Price: ${formatCurrency(i.price)} - Stock: ${i.quantity}</div>`).join('')}
            </div>
        `;
    }
    document.getElementById('ownerContent').innerHTML = html;
